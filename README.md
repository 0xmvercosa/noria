...
| Public Arbitrum adapter and factory                                | Deployed through Rabby; receipts, constructor inputs and runtime checked; configure the verified factory address in the frontend |
...
**Acceptance status:** The Graph discovery is implemented, and the Aqua lifecycle has onchain execution evidence from isolated Arbitrum forks. The adapter and factory were deployed on public Arbitrum through Rabby; [the deployment record](docs/1inch/evidence/public-deployment/README.md) documents successful receipts, matching constructor/runtime checks and launch-verifier acceptance on September 13, 2026. The hosted app must use that verified factory address; user-confirmed Privy financial-flow acceptance remains pending. A real Privy embedded wallet connection has been observed, but connection, approval, provider checkout, Rabby deployment or fork impersonation does not prove the required Privy financial flow. Shipment to Aqua does not establish aggregator admission or organic taker demand.

## The complete financial journey

1. **Create and fund the wallet — Privy.** Sign in with Google, email or a wallet and open the embedded Ethereum wallet. On `/reserve`, choose **Buy USDC with euros** to request a provider quote in EUR for native USDC on Arbitrum. The provider handles available payment methods, eligibility, exchange rate and fees. Alternatively, fund from another wallet. Keep ETH in the wallet for network fees. Noria reads actual balances from Arbitrum; closing the checkout is not proof of payment or delivery.
...
**Deployment and demo:** The [public deployment record](docs/1inch/evidence/public-deployment/README.md) identifies the existing Arbitrum adapter and factory. See the [step-by-step Rabby deployment and testing guide (PT-BR)](docs/1inch/deploy-and-test.pt-BR.md), [contract review](docs/1inch/contract-review.md), and [owner-confirmed lifecycle](docs/1inch/live-launch.md). Run `npm run validate:aqua-deployment` for the isolated-fork deployment test. For a new deployment, `npm run deploy:aqua` prepares transactions locally for separate Rabby signatures. The server has no private key or broadcaster. Reports retain exact creation inputs, receipts and runtime verification; local evidence is distinct from public deployment.

...
test("official order encodes the correct maker, Aqua hash and deterministic program", () => {
  const built = buildOfficialOrder({
    maker: "0x0000000000000000000000000000000000000011",
    lowerPriceE6: 2500_000000n,
    upperPriceE6: 3500_000000n,
    lpFeeBps: 30,
    salt: 1n,
  });
  assert.equal(keccak256(built.bytes), built.strategyHash);
  assert.equal(
    Order.decode(new HexString(built.bytes)).build().maker.toLowerCase(),

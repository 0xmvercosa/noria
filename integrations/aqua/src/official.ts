...
export function buildOfficialOrder(input: {
  ...
  if (input.lowerPriceE6 <= 0n || input.lowerPriceE6 >= input.upperPriceE6)
    throw new Error("invalid_range");
  if (
    !Number.isInteger(input.lpFeeBps) ||
    input.lpFeeBps < 1 ||
    input.lpFeeBps > 100
  )
    throw new Error("fee_out_of_policy");
  const program = AquaXYCAmmStrategy.newConcentrate({
    rawPriceMin: input.lowerPriceE6,
    rawPriceMax: input.upperPriceE6,
  })
    .withTxOriginAccessToken(new Address(DEPLOYMENTS.kycNft))
    .withFeeTokenIn(input.lpFeeBps)
    .withSalt(input.salt)
    .build();
  const order = Order.new({
    maker: new Address(input.maker),
    program,
    traits: MakerTraits.default(),
  });
  return {
    order,
    bytes: order.encode().toString() as `0x${string}`,

...
export const uint = z.string().regex(/^(0|[1-9][0-9]*)$/);
...
export type CanonicalEvidence = {
  pool: string; chainId: number; token0: string; token1: string; feeTierPips: number;
  liquidity: string; spotUSDCPerWethE6: string; blockNumber: string; blockHash: string;
  timestamp: string; mode: 'rpc' | 'synthetic-example'; canonicalFactoryPool: boolean;
};
export type CandidateAssessment = {
  candidateId: string; eligible: boolean; reasons: string[];
  historicalScorePpm: string; historicalPoolFees24hUSDCUnits: string; coverageBps: number;
};
export type PlanDecision = {
  schemaVersion: 'noria.aqua.decision.v1'; requestId: string;
  status: 'refused' | 'eligible_for_owner_review' | 'simulation_only';
  selectedCandidateId: string | null; assessments: CandidateAssessment[];
  selectedRange: Candidate['range'] | null;
  capitalUSDCUnits: string; executionChainId: typeof CHAIN_ID;
  pair: { base: typeof WETH; quote: typeof USDC };
  routingStatus: 'not_validated'; instructions: string[];
};

export function createDiscoveryRequest(capitalUSDCUnits: string, requestId: string, now = new Date()): DiscoveryRequest {
  return DiscoveryRequestSchema.parse({
    schemaVersion: 'noria.aqua.discovery.v1', requestId, createdAt: now.toISOString(),
    expiresAt: new Date(now.getTime() + 15 * 60_000).toISOString(), chainId: CHAIN_ID,
    pair: { base: WETH, quote: USDC }, capitalUSDCUnits,
    policy: { maxSourceAgeSeconds: 3600, maxCapitalShareBps: 1000, minCoverageBps: 8000, maxRangeWidthBps: 6000 },

...
Noria combines implemented Graph discovery with an Arbitrum Aqua/Aave module. The [1inch guide](1inch/README.md) and [integration specification](1inch/integration.md) are the current execution references.
...
| Wallet            | Privy external connection, account/network display and disconnect                                   | Requires public App ID, allowed origins and a real-wallet acceptance test       |
| Local user flow   | Plan download, opt-in loopback runner, full/partial reports                                         | Public UI does not sign or broadcast transactions                               |
